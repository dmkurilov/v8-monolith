...
case "$os" in
  mac)
    ...
    # macOS system C headers live inside the Xcode SDK, not /usr/include.
    # -nostdinc++ strips the C++ search paths; -isysroot restores the SDK
    # so V8's libc++ headers can #include_next the underlying C headers.
    MAC_SYSROOT="$(xcrun --show-sdk-path)"
    ...
    # 2) test_main.o — system clang with system libc++. Proves the boundary:
    #    this TU has ZERO V8 / bundled libc++ flags and only sees v8_wrapper.h.
    echo "Compiling test_main.o (system libc++ side)..."
    c++ -std=c++23 \
      -I"$PWD" \
      -c test_main.cc \
      -o "$TEST_OBJ"

    # 3) Link. System c++ pulls in system libc++ for test_main.o's std::*.
    #    V8's bundled libc++ symbols (std::__Cr::*) are already merged into
    #    libv8_monolith.a by build.sh and don't collide.
    echo "Linking v8_test..."
    c++ -o build/v8/test_build/v8_test \
